    ...
    return (
      <div className="wrapper">
        ...
        <div id="editDivPop" className="container">
          <button onClick={props.handleLessFunc} className="btn custom_button">
            hide
          </button>
...
var GetTodos = (props) => {
  ...
  function handleDay(theDay) {
    ...
    const addTodo = async () => {
      try {
        // Make HTTP POST request to the Node.js server
        console.log("tryna");
        const response = await axios.post(
          server_address + "/getTodos/" + theDay
        );
    ...
    if (doc.done) {
      ...
      return (
        <Draggable
          key={doc._id}
          draggableId={doc._id}
          index={todosOfDay.todos.indexOf(doc)}
        >
          {(provided) => (
            <div
              className="todo_div"
            ...
            >
              ...
              />{" "}
              <div onClick={() => handleEdit(doc._id, date)}>
                {doc.todoName}
              </div>
              <button
                className="btn delete_button"
