...
var GetTodos =  (props) =>{ 
var daysFormat = {long:["Monday", "Tuesday", "Wednesday","Thursday", "Friday", "Saturday", "Sunday"], 
...
short: ["M", "T", "W", "T", "F", "S", "S"]}
  ...
  var handleDragEnd = (results) =>{
    ...
    req.open("POST", "http://localhost:5000/rearrange/"+date, true);
  ...
  function clearDone(){
    let req = new XMLHttpRequest();
      req.open("POST", "http://localhost:5000/clear/"+date, true);
      req.send();
      req.onload =()=>{
  ...
  function handleAdd(){
      ...
      req.open("POST", "http://localhost:5000/todoName/"+date+"/"+input, true);
      req.send();
      req.onload =()=>{
  ...
  function deleteDoneReq(id, reqId){
    ...
    let req = new XMLHttpRequest();
    req.open("POST", "http://localhost:5000/"+reqId+date+"/"+id, true);
    req.send()
    req.onload =()=>{
      setTodos({todos:JSON.parse(req.responseText)}); 
    }
  ...
  function handleEdit(id, day){
    ...
    if(edit.state && (id === edit.id)){
      ...
      req.open("GET","http://localhost:5000/getThisTodo/"+day+"/"+id, true);
      req.send()
  ...
  function handleDay(theDay){
    ...
    setDate(theDay)
    let req = new XMLHttpRequest();
    req.open("GET", "http://localhost:5000/getTodos/"+theDay, true);
    req.send();
    console.log(theDay)
    req.onload =()=>{
      setEdit({state:false, id:null, subTasks:[], day:null})
  ...
  var handleSubAdd = ()=>{
    ...
    var req = new XMLHttpRequest()
    req.open("POST", "http://localhost:5000/getSteps/"+edit.day+"/"+edit.id+"/"+stepName, true)
    req.send()
    req.onload= () =>{
  ...
  var deleteDoneReqSub = (reqName, subName, todoID) =>{
    console.log("work")
    let req = new XMLHttpRequest();
    req.open("POST", "http://localhost:5000/"+reqName+date+"/"+todoID+"/"+subName, true);
    req.send()
    req.onload =()=>{
  ...
  let req= new XMLHttpRequest;
  req.open("GET", "http://localhost:5000/getTodos/"+date, true)
