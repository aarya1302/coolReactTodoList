...
app.get("/getTodos/:day", function (req, res) {
  ...
  db.getDB()
    ...
    .findOne({ day: req.params.day }, (err, documents) => {
      if (err) {
        console.log(err);
      } else {
          ...
          element.subTasks.forEach((sub) => {
            if (sub.stepName === req.params.subName) {
              ...
              collection.updateOne(
                ...
                {
                  $set: {
                    "todayTodos.$[elem].subTasks.$[subName].done": newVal,
                  },
                },
                {
                  arrayFilters: [
                    { "elem._id": req.params.id },
                    { "subName.stepName": req.params.subName },
                  ],
                },
                (err, Docs) => {
      ...
      docs.todayTodos.forEach((element) => {
        if (element._id + "" === req.params.id) {
          ...
          collection.updateOne(
            {
              day: req.params.day,
              todayTodos: { $elemMatch: { _id: req.params.id } },
            },
            { $set: { "todayTodos.$.done": newVal } },
            (err) => {
              if (err) {
                console.log(err);
              } else {
...
app.post("/rearrange/:day", function (req, res) {
  db.getDB()
    .collection(collection_name)
    .updateOne({ day: req.params.day }, { $set: { todayTodos: req.body } });
  db.getDB()
    .collection(collection_name)
    .findOne({ day: req.params.day }, (err, doc) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/getSteps/:day/:id/:step", function (req, res) {
  ...
  const collection = db.getDB().collection(collection_name);
  collection.updateOne(
    { day: req.params.day, todayTodos: { $elemMatch: { _id: req.params.id } } },
    {
      $push: { "todayTodos.$.subTasks": { stepName: step_name, done: false } },
    },
    (err, docs) => {
      if (err) {
        console.log(err);
...
app.get("/getThisTodo/:day/:id", function (req, res) {
  ...
  db.getDB()
    .collection(collection_name)
    .findOne({ day: req.params.day }, function (err, docs) {
      if (err) {
        console.log(err);
      } else {
        docs.todayTodos.forEach((element) => {
